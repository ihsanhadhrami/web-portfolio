  ...
  build: {
    ...
    rollupOptions: {
      output: {
         ...
         * package entry points only, so react-dom's internals landed in
         * the app entry chunk — meaning every content edit invalidated
         * ~130kB of framework code for returning visitors. Matching on
         * resolved module paths keeps vendor code in stable chunks.
         */
        manualChunks(id) {
          if (!id.includes('node_modules')) return;
          // framer-motion v12 splits its runtime across sibling packages.
